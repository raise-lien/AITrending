...
def init_db():
    ...
    # Chinese LLM summary (nullable; added for DailyBrief-style enrichment)
    cols = {r[1] for r in conn.execute("PRAGMA table_info(items)").fetchall()}
    if "zh_summary" not in cols:
        conn.execute("ALTER TABLE items ADD COLUMN zh_summary TEXT")
...
def save_digest(date: str, content: dict, model: str | None = None):
    """Upsert a daily digest JSON blob."""
    import json
    conn = get_db()
    try:
        conn.execute(
            """INSERT INTO digests (date, content, model, created_at)
               VALUES (?, ?, ?, datetime('now'))
               ON CONFLICT(date) DO UPDATE SET
                 content=excluded.content,
                 model=excluded.model,
                 created_at=datetime('now')""",
            (date, json.dumps(content, ensure_ascii=False), model),
        )
        conn.commit()
    finally:
        conn.close()
...
def get_digest(date: str):
    """Return digest dict for date, or None."""
    import json
    conn = get_db()
    try:
        row = conn.execute(
            "SELECT date, content, model, created_at FROM digests WHERE date = ?",
            (date,),
        ).fetchone()
        if not row:
            return None
...
def list_digests(limit: int = 30):
    ...
    try:
        rows = conn.execute(
            "SELECT date, model, created_at, content FROM digests ORDER BY date DESC LIMIT ?",
            (limit,),
        ).fetchall()
        out = []
        for r in rows:
